...
// Runs detection for the item's template kind and stores the result. Returns
// true when a widget was found. A miss is recorded so the inspector can say so.
export async function detectWidgetForItem(itemId: string, quiet = false): Promise<boolean> {
  ...
  if (!def.widgetKind) return false
  ...
  let crop = null
  try {
    crop = detectWidget(readPixels(img), def.widgetKind)
  } catch (e) {
    console.error('widget detection failed', e)
  }
  setWidget(itemId, { crop, notFound: !crop })
  if (!crop && !quiet) {
    toast(def.widgetKind === 'island' ? "Couldn't find an expanded Dynamic Island. Adjust it manually." : "Couldn't find a Live Activity card. Adjust it manually.")
  }
  return !!crop
}
